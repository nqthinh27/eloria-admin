...
| **BE26** | **Không biết được dòng đơn còn trả được mấy cái** — `OrderDetailResDTO` không có `returnedQuantity`/`returnableQuantity`, mà `POST /return/search` lại trả **`lines: null`** ⇒ FE phải `search({orderId})` rồi `GET /return/{id}` **từng phiếu** để cộng lại (**N+1 request**). | ✅ **ĐÃ XONG (2026-09-13)** — backend thêm **`lines[].returnedQuantity`** vào `GET /order/{id}`, đúng phương án rẻ nhất FE đề xuất. Đo thật 3 ca: phiếu `PENDING_APPROVAL` ⇒ `1` · từ chối phiếu ⇒ về `0` · đơn chỉ có phiếu `REJECTED` ⇒ `0` — **khớp đúng cách backend chặn `error.return.quantityExceeded`**. FE đã **gỡ hẳn `loadReturnable()`**: chọn đơn nay tốn **đúng 1 request** thay vì `1 + N`. |
| **BE27** | **Sort `/return/search` theo `staffName` hoặc `lines` trả HTTP 500** (field DTO-only ⇒ `PropertyReferenceException`). | ⏳ **Vẫn còn** — đo lại 2026-09-13: `staffName` ⇒ **500**, `lines` ⇒ **500**; 2 cột đó giữ `enableSorting: false`. *(`branchName` thì **200 và sắp đúng** — FE khoá cột đó chỉ vì bảo thủ, mở được khi muốn.)* ⚠️ **Đừng nhầm với "BE27" trong tài liệu backend** — mã đó bên backend chỉ nói về sort `staffName`/`branchName` của **`/work-shift/search`**, một endpoint khác. Phần work-shift backend cũng **đã code nhưng chưa deploy**: đo thật `sort=staffName` trên `/work-shift/search` **vẫn 500**. |
| **BE28** | **Báo cáo doanh thu (P7) chưa trừ hàng trả** — `report/sales` · `report/profit` · `dashboard/summary` tính trên `order_sale` COMPLETED, trong khi tiền đã hoàn nằm ở `return_request` ⇒ doanh thu bị thổi phồng. | ✅ **ĐÃ XONG (2026-09-13)** — backend thêm **field mới**, giữ nguyên nghĩa GỘP của `revenue`/`totalRevenue` để không phá màn cũ: `dashboard/summary` ⇒ `returnRefundTotal` + `revenueAfterReturns` · `report/sales` ⇒ `returnRefund`/`revenueAfterReturns` per-row + `totalReturnRefund`/`totalRevenueAfterReturns` · `branch-comparison` ⇒ 2 field per-row. Net hoàn = `Σ(refundAmount − collectAmount)` phiếu **đã quyết toán**, quy kỳ theo `refundedAt`. Đo thật: hoàn 500.000 ⇒ `revenue` giữ 6.900.000, `revenueAfterReturns` = 6.400.000. FE đã hiện ở **Dashboard**. ⚠️ **Lãi gộp vẫn CHƯA trừ hoàn** (giá vốn hàng trả chưa snapshot) ⇒ đừng suy `revenueAfterReturns − cogs`. ⚠️ Còn **BE30**. |
| **BE29** | **`keyword` của `POST /sku/search` không khớp `productName`** — chỉ khớp mã SKU + EAN ⇒ gõ "linen" trả 0 dù có 12 SKU thuộc *"Áo sơ mi linen trắng basic"*. | ✅ **ĐÃ XONG (2026-09-13)** — backend mở rộng `keyword` sang **`product.name` + `product.code`**, không đổi contract. Đo thật: `"linen"` ⇒ 12 · `"Áo sơ mi"` (có dấu) ⇒ 12 · `"SP001"` ⇒ 12 · EAN ⇒ 1. ⇒ Tra phía server **không còn kém lọc phía FE**, đúng điều kiện để ô chọn SKU luôn chạy server mode. FE chỉ đổi lại placeholder/câu báo rỗng. |
| **BE30** | 🐞 **`POST /report/sales` `groupBy=DAY` LÀM MẤT tiền hoàn khi ngày đó không có đơn** — dòng DAY chỉ sinh cho ngày **có đơn**, phiếu hoàn rơi vào ngày không bán được gì thì biến mất khỏi **cả `rows` lẫn `totalReturnRefund`**. | ⚠️ **Backend đã code xong (2026-09-13) nhưng CHƯA lên server đang chạy.** Bản sửa phát sinh **dòng riêng cho mọi key chỉ-có-hoàn** (`revenue = 0`, `returnRefund = net`, **`revenueAfterReturns` ÂM**), chèn đúng thứ tự thời gian với DAY/MONTH/YEAR và **xuống cuối** với BRANCH/PRODUCT. Đo thật trên server hiện tại **vẫn lỗi**: cùng một kỳ, `DAY` ⇒ `totalReturnRefund = 0` trong khi `MONTH`/`BRANCH`/`PRODUCT`/`YEAR` + `dashboard/summary` ⇒ **500.000**. ⇒ **Backend cần build/chạy lại.** FE **không phải sửa gì**: `formatVnd` dùng `Intl.NumberFormat` nên số âm hiện đúng, biểu đồ Dashboard vẽ theo `netRevenue` nên dòng chỉ-hoàn chỉ thêm một điểm `0`. |
...
2. **Số "còn trả được"** = `quantity - lines[].returnedQuantity` của `GET /order/{id}` (backend bổ
   sung 2026-09-13, **BE26 đã đóng** — bản đầu phải tự cộng bằng **N+1 request**). Phiếu `REJECTED`
   không tính, khớp cách backend đếm khi chặn `quantityExceeded`.
...
Chỉ còn **BE27** (sort `staffName`/`lines` trên `/return/search` ⇒ 500) — **không chặn**, 2 cột đó
đã khai `enableSorting: false`.

*Đã đóng trong ngày 2026-09-13:* **BE26** (số còn trả được ⇒ FE gỡ hẳn N+1) · **BE28** (báo cáo
doanh thu đã trừ hàng trả). ⚠️ BE28 kéo theo **BE30** — `report/sales` `groupBy=DAY` làm mất tiền
hoàn của ngày không có đơn; chưa chặn vì FE chưa có màn Báo cáo bán hàng.
